...
class DicodingEventRepository private constructor (
    ...
    fun getActiveEvents(context: Context): LiveData<Result<List<DicodingEventEntity>>> = liveData {
        emit(Result.Loading)
    ...
    fun getInactiveEvent(context: Context): LiveData<Result<List<DicodingEventEntity>>> = liveData {
        emit(Result.Loading)

        try {
            val localData = eventDao.getInactiveEvents()
